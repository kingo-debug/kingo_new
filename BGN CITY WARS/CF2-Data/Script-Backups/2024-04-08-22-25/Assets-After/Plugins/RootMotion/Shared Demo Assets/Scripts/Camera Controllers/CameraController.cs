...
namespace RootMotion {
	...
	public class CameraController : MonoBehaviour {
		...
		public void UpdateInput() {
			...
			// Cursors
			ControlFreak2.CFCursor.lockState = lockCursor? CursorLockMode.Locked: CursorLockMode.None;
			ControlFreak2.CFCursor.visible = lockCursor? false: true;

			// Should we rotate the camera?
			bool rotate = rotateAlways || (rotateOnLeftButton && ControlFreak2.CF2Input.GetMouseButton(0)) || (rotateOnRightButton && ControlFreak2.CF2Input.GetMouseButton(1)) || (rotateOnMiddleButton && ControlFreak2.CF2Input.GetMouseButton(2));

			// delta rotation
			if (rotate) {
				x += ControlFreak2.CF2Input.GetAxis("Mouse X") * rotationSensitivity;
				y = ClampAngle(y - ControlFreak2.CF2Input.GetAxis("Mouse Y") * rotationSensitivity, yMinLimit, yMaxLimit);
			}
		...
		private float zoomAdd {
			get {
				float scrollAxis = ControlFreak2.CF2Input.GetAxis("Mouse ScrollWheel");
